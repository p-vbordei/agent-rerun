...
describe("sha256Hex", () => {
  test("hashes the empty byte string to the known SHA-256 value", () => {
    ...
    expect(got).toBe("sha256:e3b0c44298fc1c149afbf4c8996fb92427ae41e4649b934ca495991b7852b855");
  });

  test("hashes ASCII bytes correctly", () => {
    const got = sha256Hex(new TextEncoder().encode("abc"));
    expect(got).toBe("sha256:ba7816bf8f01cfea414140de5dae2223b00361a396177a9cb410ff61f20015ad");
  });
...
describe("sha256OfJcs", () => {
  test("hashes JCS({}) to the known sha256 of the bytes '{}'", () => {
    ...
    expect(got).toBe("sha256:44136fa355b3678a1146ad16f7e8649e94fb4fc21fe77e8310c060f61caaff8a");
  });
